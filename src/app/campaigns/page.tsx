      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-[var(--fg)]">Campañas</h1>
              <p className="text-sm text-[var(--fg-muted)]">Gestión completa — activar, pausar, renombrar, ajustar presupuesto</p>
            </div>
            <div className="flex items-center gap-2">
              <Link
                href="/campaigns/new"
                className="flex items-center gap-2 bg-[var(--accent)] text-white px-3 py-1.5 rounded text-sm font-medium hover:opacity-90"
              >
                <Plus size={14} />
                Nueva campaña
              </Link>
          ...
          {error && (
            <div className="bg-[var(--danger)]/10 border border-[var(--danger)]/30 text-[var(--danger)] px-4 py-3 rounded text-sm">{error}</div>
          )}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.length === 0 && !loading && (
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--fg-faint)]">Sin campañas</td></tr>
                  )}
                  {rows.map((c) => {
                    ...
                    return (
                      <tr key={c.id} className="border-t border-[var(--hairline)] hover:bg-[var(--surface)]">
                        ...
                        <td className="px-4 py-2 text-xs text-[var(--fg-soft)] font-mono">{c.objective}</td>
                        ...
                        <td className="px-4 py-2">
                          <div className="flex justify-end gap-1">
                            <button
                              ...
                              className="p-1.5 rounded hover:bg-[var(--surface-2)] text-[var(--fg-soft)] transition-colors"
